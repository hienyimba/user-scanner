...
final class ScanRunStore
{
     ...
     */
    public function createRun(string $mode, array $targets, array $options = [], array $expandedTargets = []): string
    {
        $data = $this->load();
     ...
     * @param array<int,string> $expandedTargets
     */
    public function appendResults(string $runId, array $results, array $expandedTargets = []): void
    {
        $data = $this->load();
        ...
        if ($expandedTargets !== []) {
            $run['expanded_targets'] = array_values(array_unique(array_merge($run['expanded_targets'] ?? [], $expandedTargets)));
        }
