...
final class LichessValidator extends BaseGeneratedValidator
{
    ...
    {
        return "https://lichess.org/api/player/autocomplete?term={$target}&exists=1";
    }
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'lichess: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'lichess: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'lichess: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['false'];
        $takenIndicators = ['true'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'lichess: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'lichess: indeterminate email response (HTTP ' . $status . ')'];
    }
