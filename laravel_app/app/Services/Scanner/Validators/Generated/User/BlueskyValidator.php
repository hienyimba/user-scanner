...
final class BlueskyValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'bluesky: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'bluesky: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'bluesky: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['com.atproto.temp.checkhandleavailability#resultavailable'];
        $takenIndicators = ['com.atproto.temp.checkhandleavailability#resultunavailable'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'bluesky: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'bluesky: indeterminate email response (HTTP ' . $status . ')'];
    }
