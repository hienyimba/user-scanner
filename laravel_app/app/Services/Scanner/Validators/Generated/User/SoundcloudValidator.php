...
final class SoundcloudValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'soundcloud: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'soundcloud: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'soundcloud: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['[403] request forbidden try using proxy or vpn'];
        $takenIndicators = ['soundcloud://users:', 'soundcloud://users:{user}', 'username', '{user}'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'soundcloud: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'soundcloud: indeterminate email response (HTTP ' . $status . ')'];
    }
