...
final class LemmyValidator extends BaseGeneratedValidator
{
    ...
    {
        return "https://lemmy.world/api/v3/user?username={$target}";
    }
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'lemmy: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'lemmy: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'lemmy: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [400];
        $takenStatuses = [404];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'lemmy: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'lemmy: indeterminate email response (HTTP ' . $status . ')'];
    }
