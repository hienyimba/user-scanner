...
final class HashnodeValidator extends BaseGeneratedValidator
{
    ...
    {
        return "https://hashnode.com/utility/ajax/check-username";
    }
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'hashnode: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'hashnode: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'hashnode: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['status'];
        $takenIndicators = ['status'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'hashnode: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'hashnode: indeterminate email response (HTTP ' . $status . ')'];
    }
