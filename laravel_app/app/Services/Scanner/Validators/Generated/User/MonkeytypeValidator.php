...
final class MonkeytypeValidator extends BaseGeneratedValidator
{
    ...
    {
        return "https://api.monkeytype.com/users/checkName/{safe_user}";
    }
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'monkeytype: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'monkeytype: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'monkeytype: token/bootstrap extraction failure'];
            }
        }
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'monkeytype: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'monkeytype: indeterminate email response (HTTP ' . $status . ')'];
    }
