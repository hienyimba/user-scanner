...
final class StackoverflowValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'stackoverflow: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'stackoverflow: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'stackoverflow: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['no user found with matching email'];
        $takenIndicators = ['the email or password is incorrect'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'stackoverflow: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'stackoverflow: indeterminate email response (HTTP ' . $status . ')'];
    }
