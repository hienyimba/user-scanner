...
final class SaashubValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'saashub: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'saashub: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'saashub: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = [' in res_text and ', 'we couldn'];
        $takenIndicators = ['email - has already been taken'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'saashub: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'saashub: indeterminate email response (HTTP ' . $status . ')'];
    }
