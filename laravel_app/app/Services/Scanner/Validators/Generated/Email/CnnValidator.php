...
final class CnnValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'cnn: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'cnn: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'cnn: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['cnn.createprofile', 'cnn.updatepassword'];
        $takenIndicators = ['identity.already.in.use'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'cnn: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'cnn: indeterminate email response (HTTP ' . $status . ')'];
    }
