...
final class AljazeeraValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'aljazeera: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'aljazeera: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'aljazeera: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['email is available', 'not found', 'not registered'];
        $takenIndicators = ['already associated', 'already exists', 'registered'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'aljazeera: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'aljazeera: indeterminate email response (HTTP ' . $status . ')'];
    }
