...
final class AddictinggamesValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'addictinggames: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'addictinggames: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'addictinggames: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['name: the username tierd_knight is already taken'];
        $takenIndicators = ['is already taken', 'mail: the email address'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'addictinggames: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'addictinggames: indeterminate email response (HTTP ' . $status . ')'];
    }
