...
final class RenderValidator extends BaseGeneratedValidator
{
    ...
    {
        return "https://api.render.com/graphql";
    }
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'render: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'render: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'render: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['hcaptcha_token', 'invalid'];
        $takenIndicators = [')\n                if ', 'email', 'exists', 'message'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'render: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'render: indeterminate email response (HTTP ' . $status . ')'];
    }
