...
final class IndiatimesValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'indiatimes: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'indiatimes: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'indiatimes: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['unregistered_email'];
        $takenIndicators = ['unverified_email', 'verified_email'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'indiatimes: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'indiatimes: indeterminate email response (HTTP ' . $status . ')'];
    }
