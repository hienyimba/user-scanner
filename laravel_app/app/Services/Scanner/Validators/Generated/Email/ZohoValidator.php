...
final class ZohoValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'zoho: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'zoho: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'zoho: token/bootstrap extraction failure'];
            }
        }
        ...
        $takenStatuses = [200];
        $availableIndicators = [];
        $takenIndicators = ['user exists', 'user exists in another dc'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'zoho: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'zoho: indeterminate email response (HTTP ' . $status . ')'];
    }
