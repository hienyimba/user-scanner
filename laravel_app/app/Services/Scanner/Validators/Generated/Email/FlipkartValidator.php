...
final class FlipkartValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'flipkart: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'flipkart: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'flipkart: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['looks like you'];
        $takenIndicators = ['supportedauthenticationtypes=password'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'flipkart: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'flipkart: indeterminate email response (HTTP ' . $status . ')'];
    }
