...
final class FlirtbateValidator extends BaseGeneratedValidator
{
    ...
    {
        return "https://api.flirtbate.com/api/v1/customer/reset-password-email";
    }
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'flirtbate: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'flirtbate: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'flirtbate: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['email invalid for reset password'];
        $takenIndicators = ['reset password email sent'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'flirtbate: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'flirtbate: indeterminate email response (HTTP ' . $status . ')'];
    }
