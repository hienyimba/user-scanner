...
final class SexvidValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'sexvid: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'sexvid: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'sexvid: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['doesnt_exist', 'no user with such email exists'];
        $takenIndicators = ['a new generated password has been sent', 'status\\', 'success'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'sexvid: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'sexvid: indeterminate email response (HTTP ' . $status . ')'];
    }
