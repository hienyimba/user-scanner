...
final class FreelancerValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'freelancer: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'freelancer: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'freelancer: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [200];
        $takenStatuses = [200];
        $availableIndicators = [];
        $takenIndicators = ['email_already_in_use'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'freelancer: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'freelancer: indeterminate email response (HTTP ' . $status . ')'];
    }
