...
final class MadePornValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'made_porn: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'made_porn: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'madeporn: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['error', 'the email address is incorrect'];
        $takenIndicators = ['sent an email with a link', 'text'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'made_porn: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'made_porn: indeterminate email response (HTTP ' . $status . ')'];
    }
