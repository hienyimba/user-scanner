...
final class PornhubValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'pornhub: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'pornhub: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'pornhub: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['create_account_passed'];
        $takenIndicators = ['already in use', 'create_account_passed'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'pornhub: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'pornhub: indeterminate email response (HTTP ' . $status . ')'];
    }
