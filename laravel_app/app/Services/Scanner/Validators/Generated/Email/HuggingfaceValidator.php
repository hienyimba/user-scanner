...
final class HuggingfaceValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'huggingface: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'huggingface: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'huggingface: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['this email address is available.'];
        $takenIndicators = ['already exists', 'rate limited wait for few minutes'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'huggingface: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'huggingface: indeterminate email response (HTTP ' . $status . ')'];
    }
