...
final class EnvatoValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'envato: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'envato: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'envato: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [204];
        $takenStatuses = [200];
        $availableIndicators = [];
        $takenIndicators = ['already in use'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'envato: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'envato: indeterminate email response (HTTP ' . $status . ')'];
    }
