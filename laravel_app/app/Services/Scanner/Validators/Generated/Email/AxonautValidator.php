...
final class AxonautValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'axonaut: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'axonaut: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'axonaut: token/bootstrap extraction failure'];
            }
        }
        ...
        $takenStatuses = [200];
        $availableIndicators = [];
        $takenIndicators = ['/login?email', 'location'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'axonaut: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'axonaut: indeterminate email response (HTTP ' . $status . ')'];
    }
