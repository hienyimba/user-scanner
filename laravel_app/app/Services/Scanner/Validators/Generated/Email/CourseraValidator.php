...
final class CourseraValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'coursera: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'coursera: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'coursera: token/bootstrap extraction failure'];
            }
        }
        ...
        $takenStatuses = [200];
        $availableIndicators = [];
        $takenIndicators = ['unexpected data type for loginmethods: {type(methods)}, report it on github'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'coursera: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'coursera: indeterminate email response (HTTP ' . $status . ')'];
    }
