...
final class UbisoftValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'ubisoft: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'ubisoft: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'ubisoft: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['invalid email', 'legaloptinskey'];
        $takenIndicators = ['invalid email'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'ubisoft: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'ubisoft: indeterminate email response (HTTP ' . $status . ')'];
    }
