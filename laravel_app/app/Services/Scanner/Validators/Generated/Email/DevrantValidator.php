...
final class DevrantValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'devrant: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'devrant: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'devrant: token/bootstrap extraction failure'];
            }
        }
        ...
        $takenStatuses = [200];
        $availableIndicators = [];
        $takenIndicators = ['the email specified is already registered to an account.'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'devrant: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'devrant: indeterminate email response (HTTP ' . $status . ')'];
    }
