...
final class FacebookValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'facebook: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'facebook: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'facebook: token/bootstrap extraction failure'];
            }
        }

        $availableStatuses = [404];
        $takenStatuses = [200];
        $availableIndicators = ['no search results', 'your search did not return any results.'];
        $takenIndicators = ['redirectpageto', 'serverredirect'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'facebook: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'facebook: indeterminate email response (HTTP ' . $status . ')'];
    }
