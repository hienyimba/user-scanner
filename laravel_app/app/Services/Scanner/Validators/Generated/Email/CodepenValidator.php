...
final class CodepenValidator extends BaseGeneratedValidator
{
    ...
    protected function parseConnectorResponse(\Illuminate\Http\Client\Response $response, string $target): array
    {
        ...
        foreach ($blockedStatuses as $blocked) {
            if ($status === $blocked) {
                return ['Error', 'codepen: blocked/rate-limited (HTTP ' . $status . ')'];
            }
        }

        foreach (['captcha', 'challenge', 'verify you are human', 'cloudflare', 'bot check'] as $needle) {
            if (str_contains($body, $needle)) {
                return ['Error', 'codepen: anti-bot challenge detected'];
            }
        }
        ...
        foreach (['csrf', 'authenticity_token', 'x-csrf-token', 'token required', 'invalid token'] as $needle) {
            if (str_contains($body, $needle) && $status >= 400) {
                return ['Error', 'codepen: token/bootstrap extraction failure'];
            }
        }
        ...
        $takenStatuses = [200];
        $availableIndicators = [];
        $takenIndicators = ['that email is already taken.'];
        ...
        if ($this->mode() === 'username') {
            ...
            return ['Error', 'codepen: indeterminate username response (HTTP ' . $status . ')'];
        }
        ...
        return ['Error', 'codepen: indeterminate email response (HTTP ' . $status . ')'];
    }
