    ...
    public function check(string $target, array $options = []): ScanResult
    {
        try {
            ...
            $response = $request
                ...
                ->post('https://identitytoolkit.googleapis.com/v1/accounts:createAuthUri?key=AIzaSyDWz9TZvPscN7qFdG6z1bvLh98eXVBSUPM');
            ...
            $reason = str_contains($message, 'timed out')
                ? (str_contains($message, 'read') ? 'Server took too long to respond (Read Timeout)' : 'Connection timed out while reaching Al Jazeera Auth')
                : $e->getMessage();
